        ...
        <div class = "recipe_content">
            ...
            <div class = "recipe_container">
                <div id="menu-list">
                    <!-- menus are here -->
                    <?php
                    $cuisine = $_POST['cusine_dropdown'];
                    $order = $_POST['order_dropdown']; 
                    ...
                    if (isset($_GET['category'])) {
                        ...
                        switch ($category) {
                            case 'breakfast':
                                ...
                                if ($cuisine == "0" && $order == "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 1";
                                }
                                else if ($cuisine == "0" && $order != "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 1 ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                else if ($cuisine != "0" && $order == "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 1 && cuisine_type = $cuisine";
                                }
                                else{
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 1 AND cuisine_type = $cuisine ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                $result = mysqli_query($conn, $sql);
                                break;
                            ...
                            case 'lunch':
                                ...
                                if ($cuisine == "0" && $order == "none"){
                                    ...
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 2 ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                else if ($cuisine != "0" && $order == "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 2 && cuisine_type = $cuisine";
                                }
                                else{
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 2 AND cuisine_type = $cuisine ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                $result = mysqli_query($conn, $sql);
                                break;
                            ...
                            case 'dinner':
                                ...
                                if ($cuisine == "0" && $order == "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 3";
                                }
                                else if ($cuisine == "0" && $order != "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 3 ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                else if ($cuisine != "0" && $order == "none"){
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 3 && cuisine_type = $cuisine";
                                }
                                else{
                                    $sql = "SELECT RecipeID, Name, url FROM Recipe WHERE meal_type = 3 AND cuisine_type = $cuisine ORDER BY Name " . mysqli_real_escape_string($conn, $order);
                                }
                                $result = mysqli_query($conn, $sql);
                                break;
